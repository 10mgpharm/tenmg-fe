...
import React, { useEffect, useRef, useTransition, useState, Dispatch, SetStateAction } from "react";
import {
  Box,
  Text,
  FormControl,
...
import { createBankAccount, createBankMandate, getBankList, verifyBankAccount } from "../actions";
import { z } from "zod";
...
export default function StepFourBankForm({
  ...
  const handleContinue = async () => {
    const response = await createBankMandate(token, {
      duration: durationInMonth ? durationInMonth.toString() : "0",
      loanApplicationIdentifier: application.identifier,
      bank: defaultBankAccount.bankName,
      customerAccountName: defaultBankAccount.accountName,
      customerAccountNumber: defaultBankAccount.accountNumber,
      customerBankCode: defaultBankAccount.bankCode,
    });
    if (response.status === "error") {
      ...
      {(!defaultBankAccount || edit) && (
        <>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="w-full">
            ...
            <Box mb={8}>
              <Button colorScheme="purple" size="lg" w="full" type="submit" isLoading={saveBankLoading} loadingText="Saving Account Details...">
      ...
      {defaultBankAccount && !edit && (
        <>
          ...
          <Flex justifyItems={"between"} alignItems={"center"} gap={5} mt={5} mb={8}>
            ...
            <Button colorScheme="blue" size="lg" w="full" onClick={handleContinue} isLoading={saveBankLoading} loadingText="Saving Account Details...">
              Initiate Mandate
            </Button>
          </Flex>
          <Center gap={2}>
