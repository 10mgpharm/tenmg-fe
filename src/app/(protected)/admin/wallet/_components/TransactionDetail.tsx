          ...
          <Stack gap={4} mt={4} px={3} py={5} rounded={"md"} className="border">
            {
              (type === "transaction" || type === "loan-wallet" || type === "supplier-wallet") ? (
                <Flex justify={"space-between"}>
                  ...
                  <Tag 
                  colorScheme={
                    (selectedRow?.txnType === "CREDIT" || selectedRow?.type === "CREDIT" )
                    ? "green" : 
                    selectedRow?.txnType === "DEBIT" || selectedRow?.type === "DEBIT"
                    ? "red" : "orange"} fontWeight={500}
                  >
                    {(type === "transaction" || type === "supplier-wallet") ? selectedRow?.status : selectedRow?.type}
                  </Tag>
                </Flex>
              )
              : (
              ...
              type !== "sup_payout" && (
                <Flex justify={"space-between"}>
                  ...
                  <Text fontWeight={500}>
                    ...
                    selectedRow?.txnGroup === "WITHDRAW_TO_BANK" ? "Withdraw" :
                    ...
                    selectedRow?.transactionGroup === "loan_disbursement" ? "Loan Disbursement" :
                    ""
                  }
                  </Text>
                </Flex>
