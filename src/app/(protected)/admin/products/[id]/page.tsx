...
const ProductDetail = ({ params }: { params: { id: string } }) => {
  ...
  const productPrice =
    Number(products?.actualPrice) - Number(products?.discountPrice);
          ...
          <div className="flex-1 bg-white p-5 rounded-md pb-9">
              ...
              <div className="mb-2 flex items-center gap-2">
                <p
                  className={classNames(
                    products?.status === "PENDING" ||
                      ...
                      : "text-gray-500",
                    " max-w-min px-2 rounded-xl py-0.5 capitalize text-xs font-medium flex items-center gap-1"
                  )}
                >
                  <span className="text-[1.2rem] rounded-full flex items-start justify-center">
                    •
                  </span>{" "}
                  {products?.status === "APPROVED"
                    ? "ACTIVE"
                ...
                <Tag
                  ...
                  color={
                    ...
                    (products?.outStockLevel ?? 0)
                      ? "error.500"
                      : parseInt(products?.quantity) >=
                          (products?.outStockLevel ?? 0) &&
                        parseInt(products?.quantity) <= products?.lowStockLevel
                      ? "warning.500"
                      : "green.500"
                  }
                  bgColor={
            ...
            <div className="flex justify-between">
              ...
              <div className="">
                <h2 className="text-lg font-semibold text-primary-500">
                  ₦{productPrice?.toLocaleString()}.00
                </h2>
                <h2 className="text-sm font-medium text-gray-400 line-through">
                  ₦{Number(products?.actualPrice)?.toLocaleString()}.00
                </h2>
              </div>
            </div>
            <ul className="list-disc mt-5 space-y-2 list-inside ml-4">
              ...
              <li className="text-sm">
                Medication Type:
                <span className="font-semibold ml-1">
                  {products?.medicationType?.name}
                </span>
              </li>
              <li className="text-sm">
                Expiration Date:
                <span className="font-semibold ml-1">
                  {convertDate(products?.expiredAt)}
                </span>
              </li>
              <li className="text-sm">
                Created Date:
                <span className="font-semibold ml-1">
                  {convertDate(products?.createdAt)}
                </span>
              </li>
            </ul>
          </div>
