...
import TransactionSummary from "@/app/(protected)/vendors/transactions-history/_components/TransactionSummary";
  ...
  const fetchCustomerTnx = useCallback(async () => {
    ...
    try {
      const response = await requestClient({ token: token }).get(
        `admin/txn_history/creditscore-breakdown/${evaluationId}`
      );
  ...
  const downloadCustomerTnx = useCallback(async () => {
    ...
    try {
      const response = await requestClient({ token: token }).post(
        `admin/txn_history/download/${params.id}`
      );
