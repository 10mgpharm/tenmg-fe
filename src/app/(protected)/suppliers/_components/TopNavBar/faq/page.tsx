              ...
              <div className="mx-auto flex flex-col items-center justify-center gap-4">
                ...
                <div>
                  <p className="text-base my-2 text-center">
                    Nothing to see yet, checkback later
                  </p>
                </div>
              </div>
            )}
...
const FaqCard = ({ faq }) => {
  ...
  return (
    <div
      ...
      className={`my-5 w-full mx-auto flex gap-6 duration-300 rounded-md p-4 cursor-pointer ${
        open ? "bg-primary-50 max-h-fit" : "max-h-fit"
      }`}
    >
      <div>
        {open ? (
      ...
      <div className="w-full overflow-y-hidden">
        ...
        <div
          className={`transition-all duration-700 ease-in-out overflow-hidden ${
            open ? "max-h-fit translate-y-0" : "max-h-0 -translate-y-full"
          }`}
        >
          <p
            className={`pt-2 ease-in-out duration-500 ${
              open ? "translate-y-0" : "-translate-y-full text-xs"
            }`}
          >
            {faq?.answer}
          </p>
