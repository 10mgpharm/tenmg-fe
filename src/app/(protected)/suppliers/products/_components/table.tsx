...
export function ColumsProductFN(
  ...
  return [
      ...
      cell: (info) => (
        <div className="flex items-center gap-2 pl-6">
          ...
          <Link
            ...
            href={`/suppliers/products/${info.row.original.id}`}
          >
    ...
    columnHelper.accessor("actualPrice", {
      ...
      cell: (info) => {
        const productPrice =
          Number(info.row.original?.actualPrice) -
          Number(info.row.original?.discountPrice);
        return (
          <div>
            ...
            <p
              className={classNames(
                info?.row?.original?.status === "PENDING" ||
                  ...
                  : "text-gray-500",
                " max-w-min p-0.5 px-2 rounded-2xl text-[11px] capitalize font-medium"
              )}
            >
        ...
        return (
          <Flex justify={"flex-start"}>
            <Menu placement="bottom-start">
              ...
              <MenuList>
                <Link href={`/suppliers/products/${info.row.original.id}`}>
                  <MenuItem>View Product</MenuItem>
                </Link>
                <Link href={`/suppliers/products/edit/${info.row.original.id}`}>
