...
const ActivityLogs = () => {
  ...
  const fetchData = async (page: number) => {
    ...
    const url = searchValue
      ? `vendor/audit-logs?search=${searchValue}&page=${page}&limit=${ITEMS_PER_PAGE}`
      : `vendor/audit-logs?page=${page}&limit=${ITEMS_PER_PAGE}`;
  ...
  return (
    <div>
      ...
      <div className="">
        {data?.data?.length === 0 ? (
          <EmptyResult
            heading={searchValue ? `No Match Found` : `No Activity Log`}
            content={searchValue ? `` : `All Activity Logs will appear here!`}
          />
        ) : data?.data?.length > 1 ? (
