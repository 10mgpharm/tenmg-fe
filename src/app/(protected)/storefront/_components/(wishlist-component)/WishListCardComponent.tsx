      ...
      <div>
        <div className='grid grid-cols-1 lg:grid-cols-5 justify-between items-center'>
          <div className='flex items-center gap-2 lg:col-span-4 col-span-1'>
            ...
            <div>
              <h4 className='text-lg font-medium text-gray-700'>{product?.product?.name}</h4>
              <p className='text-sm  text-gray-500 my-1'>{product?.product?.description}</p>
          ...
          <div className='space-y-4 col-span-1'>
            <div className='flex items-center gap-3'>
              {product?.product?.discountPrice > 0 && (
                <p className="text-gray-900 font-semibold my-2 text-sm">
                  ₦{parseInt(product?.product?.actualPrice) - parseInt(product?.product?.discountPrice)}
                </p>
              )}
              <p className={`font-semibold my-2 text-sm ${product?.product?.discountPrice > 0 ? "text-gray-400 line-through" : "text-gray-900"}`}>
            ...
            <div className='space-x-4'>
              ...
              >Buy Now</Button>
              <Button variant={"outline"} colorScheme={"red"} size="sm" onClick={() => removeWishlistItem(product?.id, token)}>Remove</Button>
            </div>
          </div>
        </div>
      </div>
